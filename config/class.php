	...
	class pencarian extends utama
	{
		...
		class kategori extends utama
		{
			...
			function tampil_nlp()
			{
				$ambil=$this->koneksi->query("SELECT * FROM nlp ORDER BY id_nlp DESC");
		 ...
		 //proses nlp
			...
			foreach ($dataset_nlp as $key => $value) {
					$kategori_nlp = $prediction;
			 		$nama_produk = $d[1];
					$this->koneksi->query("INSERT INTO nlp (nama_produk,kategori) VALUES('$nama_produk','$kategori_nlp')");
				}
	...
	function hapusnlp($id_nlp)
	{
		$this->koneksi->query("DELETE FROM nlp WHERE id_nlp='$id_nlp'");
	}
	}
